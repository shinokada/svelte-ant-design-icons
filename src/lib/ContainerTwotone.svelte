<script lang='ts'>
  ...
  let { size = ctx.size || '24', role = ctx.role || 'img', color = ctx.color || 'currentColor', ariaLabel =  "container twotone," , class: classname, ...restProps }: Props = $props();
</script>
