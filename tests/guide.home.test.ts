...
test('Guide home page has expected h1, meta title', async ({ page }) => {
  await expect(page.getByRole('heading', { name: 'Svelte Ant Design Icons: v1' })).toBeVisible();
});

test('Guide home page has expected meta title', async ({ page }) => {
  await expect(page).toHaveTitle('Getting Started - Svelte Ant Design Icons');
});
...
test('Guide home page has expected meta og', async ({ page }) => {
  ...
  await expect(metaOgImage).toHaveAttribute(
    'content',
    'https://open-graph-vercel.vercel.app/api/svelte-ant-design-icons?title=Getting Started'
  );
});
